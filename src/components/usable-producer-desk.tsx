    ...
    <div className="space-y-5">
      <header className="relative overflow-hidden rounded-[28px] bg-ink p-6 text-white shadow-card sm:p-8">
        ...
        <div className="relative flex flex-col gap-6 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <Badge className="border-white/10 bg-white/10 text-white">Produce</Badge>
            <h1 className="mt-5 text-[38px] font-semibold tracking-[-0.05em] sm:text-[50px]">{show.name}</h1>
            <p className="mt-2 text-sm text-white/50">{show.schedule} · {workspace.items.length} links · {completed} done · {readyLinks} ready</p>
          </div>
        ...
        <CardContent className="p-5 sm:p-6">
          <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
            <div className="max-w-2xl">
              <Badge className="bg-ink text-white"><Cloud />Saved shows</Badge>
              <h2 className="mt-3 text-2xl font-semibold tracking-[-0.04em]">Save this show online, open it anywhere.</h2>
              <p className="mt-2 text-sm leading-6 text-muted-foreground">
                Build the show here, save it online, then load it on the studio iPad or any other device.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
          ...
          <div className="mt-5 grid gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(280px,.65fr)]">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="Studio key" hint="Enter once per device">
                <div className="relative">
                  ...
                  <Input
                    ...
                    placeholder={cloudStatus?.privateKeyConfigured ? "Required" : "Optional"}
                    className="pl-9"
                  />
                </div>
              </Field>
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="mt-2 flex flex-wrap gap-1.5">
                          ...
                          {item.responseGate && <Badge className="bg-violet-50 text-violet-700 text-[9px]">Response Gate</Badge>}
        ...
        <section className="space-y-3">
          <div className="flex items-end justify-between">
            <div>
              ...
              <p className="mt-1 text-[10px] text-muted-foreground">Drag to reorder</p>
            </div>
                ...
                >
                  ...
                  <button type="button" onClick={() => setSelectedId(item.id)} className="flex min-w-0 flex-1 items-center gap-3 py-3.5 pr-3.5 text-left">
                    <span className={cn("grid size-8 shrink-0 place-items-center rounded-xl font-mono text-[10px]", item.done ? "bg-success text-white" : "bg-muted text-muted-foreground")}>
                      ...
                      <span className="flex flex-wrap items-center gap-2"><span className="font-mono text-[10px] text-muted-foreground">{item.time || "No time"}</span><Badge variant="outline" className="text-[9px]">{item.type}</Badge>{item.hour && <Badge variant="outline" className="text-[9px]">{item.hour}</Badge>}{item.responseGate && <Badge className="bg-violet-50 text-violet-700 text-[9px]">Gate</Badge>}{isLinerLink(item) && <Badge className="bg-fuchsia-50 text-fuchsia-700 text-[9px]">Liner</Badge>}</span>
                ...
                {selectedReadiness && (
                  <div className={cn("rounded-[22px] border p-4", selectedReadiness.ready ? "border-success/20 bg-success-soft" : "border-amber-200 bg-amber-50")}>
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div>
                        <p className={cn("text-[10px] font-semibold uppercase tracking-[0.15em]", selectedReadiness.ready ? "text-success" : "text-amber-800")}>Link check</p>
                      </div>
